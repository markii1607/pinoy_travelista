...
include '../../../db/db.php';
	$query = mysql_query("SELECT * FROM feedback WHERE feed_row5='5'");
	$f1count = 0;
...
?>
<?php
	$query = mysql_query("SELECT * FROM feedback WHERE feed_row5='4'");
	$f2count = 0;
	while($res = mysql_fetch_assoc($query)){
	$f2count++;
	}	
?>
<?php
	$query = mysql_query("SELECT * FROM feedback WHERE feed_row5='3'");
	$f3count = 0;
	while($res = mysql_fetch_assoc($query)){
	$f3count++;
	}	
?>
<?php
	$query = mysql_query("SELECT * FROM feedback WHERE feed_row5='2'");
	$f4count = 0;
	while($res = mysql_fetch_assoc($query)){
	$f4count++;
	}	
?>
<?php
	$query = mysql_query("SELECT * FROM feedback WHERE feed_row5='1'");
	$f5count = 0;
	while($res = mysql_fetch_assoc($query)){
	$f5count++;
	}	
    ...
    <br><br>
        ...
        <script type="text/javascript" charset="utf-8">
            window.onload = function () {
                ...
                r.text(360, 80, "OVER-ALL SATISFACTION").attr({ font: "30px 'Fontin Sans', Fontin-Sans, sans-serif" });
                pie.hover(function () {
                    this.sector.stop();
