...
if(isset($_POST['post'])){
	...
	$package_desc= $_POST['package_desc'];
		...
		$sql=mysql_query("INSERT into tour_packages(name, filename, location, description, travel_time, package_inclusion, package_exclusion,
		avail, no_of_days) VALUES ('$name', '$filename', '$location',
		'$package_desc', '$travel_time', '$package_in', '$package_ex', '$avail', '$days')") or die(mysql_error());
		?>
		<script>
		alert('New Package Posted!');
		window.location.href='tourPackages.php?success';
        </script>
		<?php
		}else{
				
					$file = 'default.png';			#### COPY after lunch. Kaon muna balbin!
					$folder="uploads/";
				$sql=mysql_query	("INSERT into tour_packages(name, filename, location, description, travel_time, package_inclusion, package_exclusion,
		avail, no_of_days) VALUES ('$name', '$file', '$location',
		'$package_desc', '$travel_time', '$package_in', '$package_ex', '$avail', '$days')") or die(mysql_error());		?>
					<script>
					alert('New Package Posted!');
					window.location.href='tourPackages.php?success';
